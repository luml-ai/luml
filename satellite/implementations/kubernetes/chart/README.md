...
The satellite reaches the API through the `kubernetes` service address, but the service port is translated to the API server's own port before the policy is evaluated, so a separate policy permits that port for the satellite pod alone; model pods are not selected by it and do not mount a service-account token either. The default 6443 matches kubeadm clusters. Set `networkPolicy.apiServerPort` to the port the API server listens on, or to 0 on installations whose API server answers on 443 and need no rule of their own. A release installed before this value existed keeps the default on upgrade rather than losing the rule.
