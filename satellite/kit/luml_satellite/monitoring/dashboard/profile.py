...
from luml_satellite.monitoring.storage.query_store import ReferenceFeatureProfile, ReferenceProfile
...
def detect_model_kind(manifest: Mapping[str, Any] | None) -> str:
    ...
    if _KIND_TABULAR_TAG in producer_tags:
        return "tabular"
    if _KIND_LLM_TAG in producer_tags:
        return "llm"
    return "unknown"


def deployment_descriptor(deployment: LocalDeployment) -> dict[str, Any]:
