...
export const signInResolver = zodResolver(z.object({
    email: z.string().email({ message: 'Email is incorrect' }),
    password: z.string().min(8, { message: 'Minimum password length 8 characters' }),
}));
export const signUpResolver = zodResolver(z.object({
    username: z.string().min(3, { message: 'Username is required.' }),
    email: z.string().email('Email incorrect'),
    password: z.string().min(8, { message: 'Minimum password length 8 characters' }),
}));
export const forgotPasswordResolver = zodResolver(z.object({
    email: z.string().email({ message: 'Email is incorrect' }),
}));
export const resetPasswordResolver = zodResolver(z
    .object({
...
export const userSettingResolver = zodResolver(z.object({
    username: z.string().min(3, { message: 'Name must be more 3 characters' }),
    email: z.string().email({ message: 'Email is incorrect' }),
}));
export const userChangePasswordResolver = zodResolver(z
    .object({
    current_password: z
        .string()
        .min(8, { message: 'The password must be more than 8 characters' }),
...
export const orbitCreatorResolver = (orbitsList) => zodResolver(z.object({
    name: z
        .string()
        .min(1)
...
export const collectionCreatorResolver = zodResolver(z.object({
    description: z.string(),
    name: z.string().min(1),
    type: z.string().min(1),
}));
export const collectionEditorResolver = zodResolver(z.object({
    name: z.string().min(1),
    bucket_secret_id: z.string(),
}));
export const artifactCreateResolver = zodResolver(z.object({
    name: z.string().min(1),
    description: z.string(),
...
export const artifactEditResolver = zodResolver(z.object({
    name: z.string().min(1),
    description: z.string(),
    tags: z.array(z.string()),
}));
export const modelUploadResolver = zodResolver(z.object({
    orbit: z.string(),
    collection: z.string(),
    name: z.string().min(1),
    description: z.string(),
...
export const satellitesResolver = zodResolver(z.object({
    name: z.string().min(1),
    description: z.string().optional(),
}));
export const createSecretResolver = zodResolver(z.object({
    name: z.string().trim().min(1, 'Name is required'),
    value: z.string().trim().min(1, 'Secret value is required'),
    tags: z.array(z.string()).optional().default([]),
}));
export const updateSecretResolver = zodResolver(z.object({
...
export const createDeploymentResolver = (formData) => zodResolver(z.object({
    name: z.string().min(1),
    description: z.string().optional(),
    tags: z.array(z.string()).optional().default([]),
    ...
    satelliteFields: z.array(z.any()).superRefine((fields, ctx) => {
        formData.value.satelliteFields.forEach((field, index) => {
            const validator = combineValidators(field.validators.map(getSatelliteValidator), field.required);
