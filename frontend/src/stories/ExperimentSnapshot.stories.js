...
const createMockProvider = (options = {}) => {
    const { staticParams = [
        ...
        'dataset-1': [
            ...
            },
            {
                ...
                outputs: {
                    response: 'Quantum computing uses quantum mechanics principles like superposition and entanglement to process information in ways that classical computers cannot. Instead of traditional bits (0 or 1), quantum computers use qubits which can exist in multiple states simultaneously.',
                    tokens_used: 156,
                },
                refs: {
    ...
    }, shouldError = false, delay = 0, } = options;
    return {
        init: async () => {
            return;
        },
        getStaticParamsList: async () => {
            ...
            if (shouldError)
                throw new Error('Failed to load static params');
            return staticParams;
        },
        getDynamicMetricsNames: async () => {
            return Object.keys(dynamicMetrics);
        },
        getDynamicMetricData: async (metricName) => {
        ...
        //   return evalsList
        // },
        getTraceSpans: async (modelId, traceId) => {
            return [];
        },
        buildSpanTree: async () => {
            return [];
        },
        getTraceId: async (params) => {
        ...
        getNextEvalsByDatasetId: async (params) => {
            return evalsList?.[params.dataset_id] ?? [];
        },
        getFreshEvalsByDatasetId: async (params) => {
            return evalsList?.[params.dataset_id] ?? [];
        },
        getAllDatasetEvals: async (params) => {
            return evalsList?.[params.dataset_id] ?? [];
        },
        resetEvalsDatasetsRequestParams: async () => {
            return;
        },
        resetDatasetPage: async (datasetId) => {
            return;
        },
        getDatasetAverageScores: async (datasetId) => {
            return [];
        },
        getEvalAnnotations: async (artifactId, datasetId, evalId) => {
            return [];
        },
        getTraces: async (params) => {
            return [];
        },
        getFreshTraces: async (params) => {
            return [];
        },
        getAllTraces: async (params) => {
            return [];
        },
        resetTracesRequestParams: async (artifactId) => {
            return;
        },
        getEvalsDatasetAnnotationsSummary: async (datasetId) => {
            return {
                feedback: [],
                expectations: [],
            };
        },
        getEvalById: async (artifactId, evalId) => {
            const data = evalsList?.[artifactId]?.find((item) => item.id === evalId);
            if (!data)
                throw new Error('Eval not found');
            return data;
        },
        getSpanAnnotations: async (artifactId, traceId, spanId) => {
            return [];
        },
        getTracesAnnotationSummary: async (artifactId) => {
            return {
                feedback: [],
                expectations: [],
            };
        },
        getEvalsColumns: async (datasetId) => {
            return {
                inputs: [
        ...
        getTracesColumns: async (artifactId) => {
            return {
                attributes: [
        ...
        validateEvalsFilter: async (filters) => {
            return [];
        },
        validateTracesFilter: async (filters) => {
            return [];
        },
    };
};
...
const meta = {
    ...
    parameters: {
        ...
        docs: {
            description: {
                component: 'A component for displaying a snapshot of a model experiment. Shows static parameters, dynamic metrics, and evals results using PrimeVue components.',
            },
        },
    },
    argTypes: {
...
export const SingleModel = {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Basic view with one model. Displays static parameters (eval_dataset, eval_version, evaluation_metrics, model_type) and dynamic metrics (metric_0, metric_1, metric_4 charts) using PrimeVue Card and Chart components.',
            },
        },
    },
};
export const MultipleModels = {
    args: {
        provider: createMockProvider({
            staticParams: [
                {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Comparison of different models. Uses the StaticParametersMultiple component to display the parameters in a comparison table..',
            },
        },
    },
};
...
export const DifferentDatasetVersions = {
    args: {
        provider: createMockProvider({
            staticParams: [
                {
    ...
    parameters: {
        docs: {
            description: {
                story: 'Compare models with different dataset versions and different evaluation metrics. Shows how the component handles differences in parameters.',
            },
        },
    },
};
