...
export const useSatelliteFields = () => {
    ...
    function checkModelVersion(modelVersion, operator, value) {
        switch (operator) {
            case 'eq':
    ...
    function checkModelTags(modelTags, operator, value) {
        if (!Array.isArray(value))
            return true;
        switch (operator) {
            case 'includes':
                return value.some((pair) => Array.isArray(pair) && pair.every((tag) => modelTags.includes(tag)));
            case 'notIncludes':
                return value.every((pair) => Array.isArray(pair) && !pair.every((tag) => modelTags.includes(tag)));
        }
    }
    function checkModelVariant(modelVariant, operator, value) {
        switch (operator) {
            case 'includes':
    ...
    function shouldDisplayField(conditions = [], currentValues, modelTags, modelVersion, modelVariant) {
    ...
    function setFields(satellite, model, currentValues) {
        if (!satellite || !model) {
            fields.value = [];
            return;
        }
        ...
        fields.value = fieldsList.filter((field) => {
            return shouldDisplayField(field.conditions, currentValues, model.manifest.producer_tags, model.manifest.version || '', model.manifest.variant || '');
        });
    }
    return {
        fields,
