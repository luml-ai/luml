...
export const useArtifactUpload = () => {
    ...
    async function upload(file, name, type, description, tags, requestInfo) {
    ...
    async function uploadToBucket(data, buffer, fileName, modelName, description, tags, requestInfo) {
        try {
            progress.value = 0;
    ...
    async function getArtifactData(file, type) {
    ...
    async function getModelArtifactData(file) {
    ...
    async function getDatasetArtifactData(file) {
    ...
    async function getExperimentArtifactData(file) {
        const buffer = await file.arrayBuffer();
        const handler = new TarHandler(buffer);
        const fileIndex = handler.scan();
        const manifest = handler.getManifest();
