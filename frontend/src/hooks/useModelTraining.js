...
export const useModelTraining = (service) => {
    const toast = useToast();
    const isLoading = ref(false);
    const isTrainingSuccess = ref(false);
    const trainingData = ref(null);
    const modelsIdList = ref([]);
    const trainingModelId = ref(null);
    const modelBlob = ref(null);
    const currentTask = ref(null);
    const getTotalScore = computed(() => trainingData.value ? toPercent(trainingData.value.test_metrics.SC_SCORE) : 0);
    ...
    const getTop5Feature = computed(() => trainingData.value?.importances.filter((item, index) => index < 5) || []);
    const isTrainMode = computed(() => trainingData.value?.predicted_data_type === 'train');
    const getPredictedData = computed(() => trainingData.value ? trainingData.value.predicted_data : {});
    async function startTraining(data) {
        isLoading.value = true;
        try {
            const result = await DataProcessingWorker.startTraining(JSON.parse(JSON.stringify(data)), WEBWORKER_ROUTES_ENUM.TABULAR_TRAIN);
    ...
    async function startPredict(request) {
        isLoading.value = true;
        const route = service === 'tabular'
