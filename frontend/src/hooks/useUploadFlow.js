...
export function useUploadFlow() {
    ...
    const worktreesPendingMessage = ref(null);
    const artifactContexts = new Map();
    const activeUploads = computed(() => Array.from(uploads.value.values()));
    const failedUploads = computed(() => Array.from(uploads.value.values()).filter((u) => u.status === 'failed'));
    function _setUpload(entry) {
    ...
    async function _requestPresignedAndPost(event) {
        _setUpload({
        ...
        try {
            artifactResponse = await api.artifacts.create(event.organization_id, event.orbit_id, event.collection_id, {
    ...
    async function handleUploadCompleted(data) {
        ...
        if (ctx) {
            try {
                await api.artifacts.update(ctx.organizationId, ctx.orbitId, ctx.collectionId, ctx.artifactId, { status: ArtifactStatusEnum.uploaded });
            }
            catch {
    ...
    function handleWorktreesPendingUpload(data) {
        worktreesPendingMessage.value =
            data.message ?? 'Worktrees will be cleaned up after uploads complete';
    }
    async function retryUpload(uploadId, event) {
        await _requestPresignedAndPost(event);
    }
    async function resumePendingUploads(runId, collectionId, organizationId, orbitId) {
        let pending;
        try {
    ...
    function getNodeArtifact(nodeId) {
