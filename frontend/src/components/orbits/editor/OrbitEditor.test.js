    ...
    beforeEach(() => {
        ...
        wrapper = mount(OrbitSettingsDialog, {
            global: {
                ...
                stubs: {
                    Dialog: {
                        template: '<div v-if="visible"><slot name="header"></slot><slot></slot><slot name="footer"></slot></div>',
                        props: ['visible', 'position', 'draggable', 'pt'],
                    },
                    Form: {
                        template: '<form id="orbit-edit-form" @submit="$emit(\'submit\', { valid: true })"><slot></slot></form>',
                        props: ['id', 'initialValues', 'resolver'],
                    },
                    InputText: {
                        template: '<input :id="id" :name="name" v-model="modelValue" @input="$emit(\'update:modelValue\', $event.target.value)" />',
                        props: ['id', 'name', 'modelValue'],
                        emits: ['update:modelValue'],
                    },
                    Select: {
                        template: '<select :id="id" :name="name" :disabled="disabled"><option v-for="opt in options" :key="opt[optionValue]" :value="opt[optionValue]">{{ opt[optionLabel] }}</option></select>',
                        props: [
                            'options',
                            'optionLabel',
                            'optionValue',
                    ...
                    Button: {
                        template: '<button :type="type" :form="form" :disabled="disabled || loading" :variant="variant" :severity="severity"><slot></slot></button>',
