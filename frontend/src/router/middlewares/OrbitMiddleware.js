...
function toSetup(next, tab) {
    return next({ name: 'setup', query: { tab } });
}
export async function orbitMiddleware(to, from, next) {
    const authStore = useAuthStore();
    const orbitsStore = useOrbitsStore();
    const organizationStore = useOrganizationStore();
    const targetTab = ROUTE_TO_TAB[to.name] ?? 'registry';
    if (!authStore.isAuth) {
    ...
    const urlOrgId = to.params.organizationId;
...
export async function setupGuard(to, from, next) {
    const authStore = useAuthStore();
    const organizationStore = useOrganizationStore();
    const orbitsStore = useOrbitsStore();
    ...
    const tab = to.query.tab ?? 'registry';
