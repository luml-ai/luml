...
import BackendOffline from '@/components/data-agent/BackendOffline.vue'
import BackendIndicator from '@/components/data-agent/BackendIndicator.vue'
import NewRepositoryDialog from '@/components/data-agent/NewRepositoryDialog.vue'
import NewItemDialog from '@/components/data-agent/NewItemDialog.vue'
import TerminalPanel from '@/components/data-agent/TerminalPanel.vue'
import BoardView from '@/pages/data-agent/BoardView.vue'
import RepositoriesView from '@/pages/data-agent/RepositoriesView.vue'
...
const isDetailRoute = computed(() =>
  route.name === 'data-agent-task' || route.name === 'data-agent-run',
)

const activeTab = ref(route.name === 'data-agent-repos' ? 'repositories' : 'board')

watch(() => route.name, (name) => {
  if (name === 'data-agent-board') activeTab.value = 'board'
  else if (name === 'data-agent-repos') activeTab.value = 'repositories'
})

watch(activeTab, (tab) => {
  const target = tab === 'repositories' ? 'data-agent-repos' : 'data-agent-board'
...
<template>
  <div class="data-agent-page" :class="{ 'is-detail': isDetailRoute }">
...
.data-agent-page :deep(.p-tabpanels) {
  flex: 1;
  display: flex;
  flex-direction: column;
  overflow: hidden;
...
.data-agent-page :deep(.p-tabpanel) {
  flex: 1;
  display: flex;
  flex-direction: column;
  overflow: hidden;
