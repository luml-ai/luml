export const dashboardFinishConfirmOptions = (accept) => ({
    message: "Before exiting, please ensure you've downloaded your predictions.",
    header: 'Are you sure?',
    rejectProps: {
        label: 'cancel',
        outlined: true,
    },
    acceptProps: {
        label: 'exit',
    },
    accept,
});
export const deleteAccountConfirmOptions = (accept) => ({
    message: 'Deleting your account is permanent and irreversible. ',
    header: 'Are you sure?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete account',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const runOptimizationConfirmOptions = (accept) => ({
    message: "Please confirm that you've reviewed all settings before proceeding.",
    header: 'Ready to start optimization?',
    rejectProps: {
        label: 'cancel',
        severity: 'secondary',
    },
    acceptProps: {
        label: 'confirm and run',
    },
    accept,
});
export const leaveOrganizationConfirmOptions = (accept) => ({
    message: 'You’ll lose access to all workspaces and data within this organization. This action can’t be undone.',
    header: 'Leave this organization?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'leave',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteUserConfirmOptions = (accept, message) => ({
    message: message || 'Deleting this account is a permanent action and cannot be reversed.',
    header: 'Delete this user?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const removeOrganizationUserConfirmOptions = (accept) => ({
    message: 'The user will lose access to all resources and data associated with your organization.',
    header: 'Remove user from organization?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteOrbitConfirmOptions = (accept) => ({
    message: 'Deleting this orbit is a permanent action and cannot be reversed.',
    header: 'Delete orbit?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteCollectionConfirmOptions = (accept) => ({
    message: 'This action is permanent and cannot be undone.',
    header: 'Delete collection?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteArtifactConfirmOptions = (accept, count) => ({
    message: 'This action is permanent and cannot be undone.',
    header: count > 1 ? `Delete ${count}  artifacts?` : 'Delete artifact?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: count > 1 ? 'delete artifacts' : 'delete artifact',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteBucketConfirmOptions = (accept) => ({
    message: 'This bucket and all its contents will be deleted. It will also be removed from all Orbits where it is used.',
    header: 'Delete bucket?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteAPIKeyConfirmOptions = (accept) => ({
    message: 'This key will be permanently deleted. You’ll still be able to generate a new one at any time.',
    header: 'Delete API key?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const deleteSecretConfirmation = {
    message: 'Ensure no active deployments are using this key, otherwise deletion will fail.',
    header: 'Delete key?',
    acceptLabel: 'delete key',
    rejectLabel: 'cancel',
    acceptProps: {
        severity: 'warn',
        variant: 'outlined',
    },
};
export const deleteRepositoryConfirmOptions = (accept, name) => ({
    message: `This will permanently delete "${name}" and all its tasks and workflows.`,
    header: 'Delete repository?',
    rejectProps: {
        label: 'cancel',
    },
    acceptProps: {
        label: 'delete',
        severity: 'warn',
        outlined: true,
    },
    accept,
});
export const leavePageConfirmOptions = (accept) => ({
    message: 'Are you sure you want to exit this page?',
    header: 'Are you sure?',
    rejectProps: {
        label: 'cancel',
        outlined: true,
    },
    acceptProps: {
        label: 'exit',
    },
    accept,
});
