...
function getAverageScore(evals: EvalsInfo[]) {
  ...
  for (const item of evals) {
    ...
    for (const key in item.scores) {
      ...
      if (typeof val === 'number') {
        const currentSum = sums[modelId]
        currentSum[key] = (sums[modelId][key] ?? 0) + val
        const currentCount = counts[modelId]
        if (currentCount) {
  ...
  return Object.keys(sums).map((modelId) => {
    ...
    const scores = Object.keys(currentSum).map((scoreName) => {
      const score = currentSum[scoreName] || 0
      const count = counts[modelId]?.[scoreName] || 0
      return {
        name: scoreName,
        value: score / count,
      }
