...
from luml_api._types import Orbit, is_uuid
...
class OrbitResource(OrbitResourceBase):
    ...
    def _get_by_id(self, orbit_id: str) -> Orbit:
        response = self._client.get(
            f"/api/v1/organizations/{self._client.organization}/orbits/{orbit_id}"
        )
    ...
    def list(self) -> list[Orbit]:
        ...
        """
        response = self._client.get(
            f"/api/v1/organizations/{self._client.organization}/orbits"
        )
        if response is None:
            return []
    ...
    def update(
        ...
        response = self._client.patch(
            f"/api/v1/organizations/{self._client.organization}/orbits/{self._client.orbit}",
