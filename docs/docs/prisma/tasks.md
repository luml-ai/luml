...
Once started, the task moves into the running lane and stays there until the agent process exits. Prisma does not interpret the agent's output; the task is considered finished when the underlying CLI process terminates. The task lands in the completed lane regardless of how it ended, and the resulting branch can then be inspected, edited, or merged.
