...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.PrimaryKeyConstraint("id"),
        sa.UniqueConstraint(
            "track_id", "name", name="uq_track_stages_track_id_name"
        ),
    )
    op.create_index(
        op.f("ix_track_stages_track_id"),
        "track_stages",
        ["track_id"],
        unique=False,
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["stage_id"], ["track_stages.id"], ondelete="SET NULL"
        ),
    ...
    op.create_index(
        op.f("ix_track_entries_track_id"),
        "track_entries",
        ["track_id"],
        unique=False,
    )
    ...
    op.execute(
        "CREATE UNIQUE INDEX uq_track_entries_track_id_stage_id "
        "ON track_entries (track_id, stage_id) WHERE stage_id IS NOT NULL"
    )


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS uq_track_entries_track_id_stage_id")
