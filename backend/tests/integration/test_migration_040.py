...
async def deployment_at_039(
    ...
    await _migrate(data.engine, "040", upgrade=True)


@pytest.mark.asyncio
async def test_upgrade_replaces_json_null_with_an_empty_object(
    deployment_at_039: tuple[AsyncEngine, uuid.UUID],
) -> None:
    engine, deployment_id = deployment_at_039
    await _set_raw(engine, deployment_id, "null")

    await _migrate(engine, "040", upgrade=True)
