    ...
    async def get_organization_orbits(self, organization_id: ShortUUID) -> list[Orbit]:
        async with self._get_session() as session:
            db_orbits = await self.get_models_where(
                session,
                OrbitOrm,
                OrbitOrm.organization_id == ShortUUID(organization_id).to_uuid(),
            )
    ...
    async def get_organization_orbits_for_user(
        ...
        async with self._get_session() as session:
            result = await session.execute(
                ...
                .where(
                    OrbitOrm.organization_id == ShortUUID(organization_id).to_uuid(),
                    OrbitMembersOrm.user_id == user_id.to_uuid(),
                )
            )
    ...
    async def get_orbit(
        ...
        async with self._get_session() as session:
            db_orbit = await self.get_model_where(
                session,
                OrbitOrm,
                OrbitOrm.id == ShortUUID(orbit_id).to_uuid(),
                OrbitOrm.organization_id == ShortUUID(organization_id).to_uuid(),
                options=[
        ...
        async with self._get_session() as session:
            ...
            if orbit.members:
                await self.create_models(
                    session,
                    OrbitMembersOrm,
                    convert_orbit_simple_members(ShortUUID(db_orbit.id), orbit.members),
                )

            return await self.get_orbit(ShortUUID(db_orbit.id), organization_id)
    ...
    async def get_orbit_members(self, orbit_id: ShortUUID) -> list[OrbitMember]:
        async with self._get_session() as session:
            db_members = await self.get_models_where(
                session,
                OrbitMembersOrm,
                OrbitMembersOrm.orbit_id == ShortUUID(orbit_id).to_uuid(),
                order_by=[
                    case(
