...
class AppService(FastAPI):
    def __init__(self, *args, **kwargs) -> None:  # type: ignore
        ...
        self.include_router(router=auth_router, prefix="/api/v1")
        self.include_router(router=email_routers, prefix="/api/v1")
        self.include_router(router=users_routers, prefix="/api/v1")
        self.include_router(router=organization_router, prefix="/api/v1")
        self.include_router(router=organization_all_routers, prefix="/api/v1")
        self.include_router(router=api_key_validate_router, prefix="/api/v1")
        self.include_router(router=satellite_worker_router)
        self.include_router(router=bucket_secret_urls_router, prefix="/api/v1")
