...
class TrackArtifactOrm(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "track_id",
            "artifact_id",
            name="uq_track_entries_track_id_artifact_id",
        ),
        Index(
    ...
    stage: Mapped[TrackStageOrm | None] = relationship(
        "TrackStageOrm", back_populates="entries", lazy="selectin"
    )

    def __repr__(self) -> str:
        return (
            f"TrackArtifact(id={self.id!r}, version={self.version!r})"
        )
...
class TrackOrm(TimestampMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "orbit_id", "name", name="uq_tracks_orbit_id_name"
        ),
    )
