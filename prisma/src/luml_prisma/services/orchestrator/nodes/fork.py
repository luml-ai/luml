...
from luml_prisma.services.orchestrator.prompts import build_fork_prompt
...
def _prepare_fork_workspace(worktree_path: str) -> Path:
    for stale in ("fork.json", "result.json"):
        stale_path = Path(worktree_path) / ".prisma" / stale
...
def _collect_proposals(
    ...
    try:
        ...
        if fork_objects is not None:
            ...
            logger.info(
                "Fork node %s: read %d proposals from .prisma/fork.json",
                node_id, len(proposals),
            )
    except Exception:
        logger.warning(
            "Fork node %s: error reading .prisma/fork.json",
...
def _read_fork_json(worktree_path: str, max_count: int) -> list[dict[str, Any]] | None:
    path = Path(worktree_path) / ".prisma" / "fork.json"
    if not path.exists():
        return None
    with open(path) as f:
        data = json.load(f)
