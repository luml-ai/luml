...
_GUIDE_REF = (
    "Refer to `.prisma/guide.md` in the worktree root "
...
`.prisma/artifact.luml` using \
`save_sklearn(model, X, path=".prisma/artifact.luml")` \
...
and `.prisma/result.json` from prior runs before defining a new metric.
4. Report the metric in `.prisma/result.json` under `"metrics"` \
...
- `.prisma/result.json` with metrics and experiment IDs (see guide.md)
- `.prisma/artifact.luml` — the LUML model artifact \
(e.g. `save_sklearn(model, X, path=".prisma/artifact.luml")`)
...
Write ONLY `.prisma/fork.json` in the worktree root. \
