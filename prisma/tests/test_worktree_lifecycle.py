...
from luml_prisma.services.merge import merge_branch
from luml_prisma.services.orchestrator.engine import (
    OrchestratorEngine,
...
class TestWorktreeCleanupOnMerge:
    ...
    async def test_cleanup_error_does_not_fail_merge(
        ...
        with patch(
            "luml_prisma.handlers.run.remove_worktree",
...
class TestAutoCommit:
    ...
    async def test_auto_commit_dirty_worktree(
        ...
        assert "prisma: auto-commit" in result.stdout
...
class TestSchedulerResilience:
    ...
    async def test_recoverable_error_continues(
        ...
        with patch(
            "luml_prisma.services.orchestrator.engine.logger",
        ):
            try:
                await eng._schedule_tick()
